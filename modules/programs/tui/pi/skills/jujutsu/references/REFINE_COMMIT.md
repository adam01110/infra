...
both descriptions nonempty? bare command opens combine-description editor. agent
must pass inline message:
...
`jj split` interactive; agent must not use. move selected changes with
`jj restore`, then create separate commits manually.

## other tools

```bash
jj absorb                            # distribute lines to prior owning commits
jj absorb -i                         # interactive; agent must avoid
...
0.44 adds interactive absorb. human may use `-i`; agent must not. mutation done?
`jj st`. unexpected? `jj undo` before manual repair.
